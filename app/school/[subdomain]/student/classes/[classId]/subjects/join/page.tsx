...
import { BookPlus, ArrowLeft, Loader2, CheckCircle2, AlertCircle } from "lucide-react";
import { studentJoinSubject } from "@/lib/ecosystem-api/studentClassesApi";
...
export default function JoinSubject() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!code.trim()) return setStatus({ type: 'error', msg: "Please enter a subject code." });

    setLoading(true);
    setStatus({ type: null, msg: "" });

    try {
      const subject = await studentJoinSubject(code.trim());
      setStatus({ type: 'success', msg: `Successfully joined "${subject.name}"!` });
      setCode("");
      setTimeout(() => router.push(`${base}/classes/${classId}/subjects`), 1500);
    } catch (err: any) {
      setStatus({
        type: 'error',
        msg: err.response?.data?.detail || "Invalid subject code. Please try again.",
      });
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
            Add <span className="text-indigo-500">Subject</span>
          </h1>
          <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">
            Enter the subject access code to unlock your learning content.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            {loading ? <Loader2 className="animate-spin" size={20} /> : "Join Subject"}
          </button>
        </form>
